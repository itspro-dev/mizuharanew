# Credit To Mikey Sano ( @Mikey_Sano_Kun) )
# Module = GabiBraunRobot
import os
import io
import requests
import shutil 
...
from ShinobuRobot import OWNER_ID, SUPPORT_CHAT
from ShinobuRobot.events import register
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./ShinobuRobot/resources/*")
    ...
    fname = "luna.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by [Ayane✨](https://t.me/AyaneRobot)")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report @YourSupport')
