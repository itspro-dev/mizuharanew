...
def pp(update: Update, context: CallbackContext):
    ...
    if reply:
        ...
        msg.reply_markdown(
            "Please reply to a sticker, or an image to search it!\nDo you know that you can search an image with a link too? `/pp [picturelink] <amount>`."
        )
        return
...
PP_HANDLER = DisableAbleCommandHandler(
    "pp", pp, pass_args=True, admin_ok=True, run_async=True
)
